...
function Voting({gameInfo, playerInfo, userId}) {
    ...
    const submitSongs = async (playerVotedFor) => {
        const { error } = await supabase.from('games').update({ scores: {
            ...gameInfo().scores,
            [userId()]: {...gameInfo().scores[userId()], [gameInfo().currentsong]: playerVotedFor}
    ...
    return (
        ...
        <Show when={!voted()} fallback={<>Waiting for other players to vote...</>}>
        <div className="flex flex-col justify-center">
            {playerInfo().map((player) => {
                return (
                    <button className="flex justify-center m-2"
                    onClick={() => {
